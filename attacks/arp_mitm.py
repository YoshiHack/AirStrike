...
def allow_ip_forwarding():
    """Enable IP forwarding. Creates /etc/sysctl.conf if missing."""
    ...
    if not os.path.exists(sysctl_conf):
        print(f"{sysctl_conf} not found. Creating a minimal sysctl.conf file...")
        with open(sysctl_conf, "w") as f:
            f.write("net.ipv4.ip_forward = 1\n")
    
    # Reload sysctl settings from the config file
    subprocess.run(["sysctl", "-p", sysctl_conf], check=True)
    print("IP forwarding enabled and settings reloaded.")
